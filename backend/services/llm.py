...
- **제공되지 않은 데이터(예: VIX 지수, 캔들 패턴, 익절/손절 비율, 수급 데이터 등)를 절대 언급하거나 추측하지 마십시오.** 오직 전달된 JSON 데이터 컨텍스트에 포함된 수치만을 근거로 삼아야 합니다.
...
- 제공된 데이터에 기반한 분석만 작성하세요 (추측 및 외부 데이터 참조 절대 금지).
"""
