...
## 섹션별 작성 지침
1. `executive_summary`: 리포트의 결론. 차트들의 상관관계를 분석하여 투자의견(BUY/HOLD/REDUCE)이 도출된 핵심 논리를 **4~5문장**으로 서술하십시오.
2. `fundamental_analysis`: 매출/영업이익 추세 차트 해석. x, y축의 주요 변곡점 위주로 간결하게 설명하십시오.
3. `valuation_analysis`: 게이지 지표 해석. PEG/ROE/유동비율이 시사하는 현재의 가치 상태를 한 문장씩 임팩트 있게 서술하십시오.
4. `technical_analysis`: RSI/이평선 차트 해석. 현재의 매매 심리와 장기 추세 방향성만 서술하십시오.
5. `risk_analysis`: MDD/변동성 차트 해석. 통계적 손실 위험과 핵심 경계 요인을 짧고 명확하게 제시하십시오.

## 중요 규칙
- 문장 간의 논리적 연결을 강화하되, 장황한 수식어는 배제하고 '결론 중심'으로 작성하십시오.
