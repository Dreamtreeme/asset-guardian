...
## 섹션별 상세 작성 가이드 (프론트엔드 차트 연동)
각 필드는 프론트엔드 UI의 특정 차트 옆에 배치됩니다. 해당 차트의 수치를 활용하여 '전문가의 해설'을 제공하십시오.

1. **`fundamental_analysis`** (매출/영업이익 추세 차트용):
   - 최근 5분기 매출액의 성장 기울기와 영업이익률의 회복/둔화 흐름을 언급하세요.
   - 예: "최근 5개 분기 매출 추이를 보면..."

2. **`valuation_analysis`** (PEG/ROE/유동비율 게이지용):
   - PEG가 시사하는 저평가 여부와 ROE의 수익 창출 효율성, 유동비율의 안정성을 연결해 설명하세요.

3. **`technical_analysis`** (RSI 및 이동평균선 차트용):
   - RSI 지수의 현재 위치(과매수/과매도)와 장기 이평선(200일/300일)의 기울기가 시사하는 추세를 분석하세요.

4. **`risk_analysis`** (MDD/수익률 분포/VaR 차트용):
   - 최대 낙폭(MDD)의 역사적 크기와 VaR 기반의 일간 손실 위험도를 기반으로 보수적 대응 전략을 제시하세요.

5. **`executive_summary`**:
   - 위 모든 차트를 종합한 핵심 요약을 **4문장 이내**로 서술하세요.

6. **`report_markdown`**:
   - 위 개별 섹션들을 논리적으로 연결하여 하나의 완성된 리서치 페이퍼 형태로 통합하십시오.
